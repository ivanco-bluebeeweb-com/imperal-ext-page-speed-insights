...
## 2026-08-19 — Plausible Scenario Testing (PST) — 2 непокрытые IPC-функции закрыты

Полный метод и детали — в `SCENARIO_TESTS.md` этого приложения. Кратко:
все 11 `@chat.function` уже вызывались хотя бы раз, но обе `@ext.expose`
IPC-поверхности для SEO Audit Engine (`ping`, `check_site_speed_ipc`)
никогда не тестировались — закрыты 6 новыми тестами в
`tests/test_pst_scenarios.py`. Полный набор (62 теста) зелёный. Реальных
багов не найдено.
